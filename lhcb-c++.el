...
;;======================================================================
;;-- Auto-insert in empty buffers : Implementation files
;;======================================================================
;; (define-auto-insert "\\.\\([Cc]\\|cc\\|cpp\\)\\'" (quote 
;; (
;;  (file-name-nondirectory (file-name-sans-extension (buffer-file-name)))

;; (let ((file-name (file-name-nondirectory (file-name-sans-extension 
;; 					  (buffer-file-name)))))
;;   (setq file-type (upcase (read-string
;;    "Is it an Algorithm, DaVinciAlgorithm, Tool or a simple class A/D/T/[no] : "))
;;           class-name file-name )
;;   (setq is-algorithm (string= "A" file-type)
;;         is-DValg     (string= "D" file-type)
;;         is-tool      (string= "T" file-type)
;;         indent-class-name
;;         (substring "                                                                                                            " 0 
;;                    (+ (length class-name) (length class-name) 4 ) ) )
...
;;   (setq interface-name "" ) )
;; (if is-algorithm (let ((alg-type (upcase (read-string "Normal, Histo or Tuple Algorithm [N]/H/T : "))))
;; 		   (setq alg-type-name "GaudiAlgorithm" )
;; 		   (if (string= "H" alg-type) (setq alg-type-name "GaudiHistoAlg"))
;; 		   (if (string= "T" alg-type) (setq alg-type-name "GaudiTupleAlg"))
;; 		   (insert " " )
;; 		   )
;; )
;; (if is-DValg (let ((alg-type (upcase (read-string "Normal, Histo or Tuple DaVinciAlgorithm [N]/H/T : "))))
;; 	       (setq alg-type-name "DaVinciAlgorithm" )
;; 	       (if (string= "H" alg-type) (setq alg-type-name "DaVinciHistoAlgorithm"))
;; 	       (if (string= "T" alg-type) (setq alg-type-name "DaVinciTupleAlgorithm"))
;; 	       (insert " " )
;; 	       )
;; ) 

;; (if (or is-algorithm is-DValg) (insert  "// from Gaudi
;; #include \"GaudiKernel/AlgFactory.h\" "))

;; (if is-tool (insert  "// from Gaudi
...
;; //-----------------------------------------------------------------------------
;; // Implementation file for class : " class-name 
;; "
;; //
...
;; (if (or is-algorithm is-DValg) (concat
;; "
;; // Declaration of the Algorithm Factory
;; DECLARE_ALGORITHM_FACTORY( " class-name " )
...
;; DECLARE_TOOL_FACTORY( " class-name " )
...
;; )
;; (if is-algorithm (concat "const std::string& name,
;; " indent-class-name "ISvcLocator* pSvcLocator)
;;   : " alg-type-name " ( name , pSvcLocator )
;; {
;; "))

;; (if is-DValg (concat "const std::string& name,
;; " indent-class-name "ISvcLocator* pSvcLocator)
;;   : " alg-type-name " ( name , pSvcLocator )
;; {
;; "))

;; (if is-tool (concat "const std::string& type,
;; " indent-class-name "const std::string& name,
;; " indent-class-name "const IInterface* parent )
;;   : GaudiTool ( type, name , parent )
;; {
;; " ))
    
;; (if (not (or is-algorithm is-DValg is-tool)) (concat " ) {
;; " ))

;; (if (not (equal "" interface-name)) (concat 
;; "  declareInterface<" interface-name ">(this);
;; "))

;; (if (and is-tool (equal "" interface-name)) (concat 
;; "  declareInterface<" class-name ">(this);
;; "))
;; _ 
...
;; // Initialization
;; //=============================================================================
;; StatusCode " class-name "::initialize() {
;;   StatusCode sc = " alg-type-name "::initialize(); // must be executed first
...
;; // Main execution
;; //=============================================================================
;; StatusCode " class-name "::execute() {

;;   if ( msgLevel(MSG::DEBUG) ) debug() << \"==> Execute\" << endmsg;

;; "))

;; (if is-DValg  (concat "  setFilterPassed(true);  // Mandatory. Set to true if event is accepted.
;; "))
...
;; //  Finalize
;; //=============================================================================
;; StatusCode " class-name "::finalize() {

;;   if ( msgLevel(MSG::DEBUG) ) debug() << \"==> Finalize\" << endmsg;

;;   return " alg-type-name "::finalize();  // must be called after all other actions
;; }
;; "
;; ))
